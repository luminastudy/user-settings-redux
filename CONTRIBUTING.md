# Contributing to @lumina-study/user-settings-redux
...
- **ESLint** with `eslint-config-agent` and `eslint-config-publishable-package-json` for linting
...
```
feat(hooks): add useRegisteredDegree hook
fix(persistence): handle edge case in storage
